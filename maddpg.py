...
from utils import save_to_txt, flatten
...
class MADDPGAgent(object):
    ...
    def __init__(
        ...
        random_seed, dirname, model_path=None, eval_mode=False):
        ...
        self.local_critic = Critic(
            ...
            random_seed, fc1_units=FC1, fc2_units=FC2).to(device)
        self.target_critic = Critic(
            self.state_size * num_agents, self.action_size * num_agents,
            random_seed, fc1_units=FC1, fc2_units=FC2).to(device)
        # Optimizer
        self.critic_optimizer = optim.Adam(
            self.local_critic.parameters(), lr=LR_CRITIC,
        ...
        self.noise = OUNoise(
            self.action_size, random_seed, sigma=SIGMA)
        self.learn_step = 0
    ...
    def learn(self, agents, experience, gamma):
        ...
        if self.learn_step == 0:
            ...
            self.soft_update(
                self.local_critic, self.target_critic, TAU
            )
            self.soft_update(
                self.local_actor, self.target_actor, TAU
            )
...
class MADDPGAgentTrainer(): 
    ...
    def __init__(
        ...
        dirname, model_path=None, eval_mode=False):
        ...
        Parameters:    
            ...
            agent_index (int): index (id) of current agent
            random_seed (int): random seed
            dirname (string): output directory to store models
            model_path (string): if defined, load saved model to resume training
            eval_mode (bool): whether to use eval mode
        """
        self.state_size = state_size
        ...
        self.agents = [
            MADDPGAgent(
                ...
                random_seed=random_seed, dirname=dirname, model_path=model_path
            ) 
    ...
    def step(self, states, actions, rewards, next_states, dones):
        ...
        if len(self.memory) < BATCH_SIZE:
            return
        if not self.learn_step % LEARN_STEP == 0:
            return
